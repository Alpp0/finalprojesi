...
class HomeController extends Controller
{
    public function index()
    {
        if (Auth::id()) {
            ...
            $post = Post::where('post_staus', '=', 'active');
    ...
    public function my_post_del($id)
    {
        ...
        $data = Post::find($id);
        $data->delete();
        return redirect()->back()->width('message', 'Post deleted Succesfully');
    }
    ...
    public function update_post_data(Request $request, $id)
    {
        ...
        if ($image) {
            $imagename = time() . '.' . $image->getClientOriginalExtension();
            $image->move('postimage', $imagename); // Yeni resmi kaydet
            $data->image = $imagename; // Veritabanındaki resim sütununu güncelle
        }
